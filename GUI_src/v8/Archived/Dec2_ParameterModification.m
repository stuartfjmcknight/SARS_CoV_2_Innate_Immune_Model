...
%addpath('Models'); addpath('SpeciesParameterConventions/')

%sbioloadproject(app.ParameterModelNameEditField.Value);
Model = app.Model;
...
switch app.ParameterModificationDropDown.Value
    ...
    case "New Reaction Parameter Sheet"
        ...
        T = table(Reaction_Name,Reaction_Index, RateConstantName, RateEquation, ChemicalEquation, Value, Units); % Build the table that will be exported.
    ...
    case "Update Reaction Parameters"
        ...
        if nargin < 6 %UNMUTE TO RUN AS FXN
            warning('Must provide at least spreadsheet name')
        end
        %
        if nargin < 5
        ...
        if nargin < 4
            ...
            RateConstantColumnIndex = 6; %column which has rate constants
            RateConstantName = 3; %column which has parameter name
            UnitsIndex = 7; %column which has the units for the rate parameter
        end
