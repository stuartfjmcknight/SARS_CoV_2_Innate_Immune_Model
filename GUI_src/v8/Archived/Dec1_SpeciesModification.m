...
switch app.SpeciesModificationDropDown.Value
    ...
    case "New Species Parameter Sheet"
        ...
        T = table(Species_Index,Species_Name,Model_Name,Initial_Amount,Units,Constant_Amount,...
    ...
    case "Update Species Parameters"
        ...
        IC = str2double(table2array(data_table(:,InitialAmountRow))); % Exctract the initial condition column from the data set
        ...
        for i = 1:length(SpeciesIndex) % Loop over all of the species
            if strcmp(ModelName(i),string(Model.Species(i).Name)) %compare that the name in the spreadsheet matches the name in the model
                ...
                %Model.Species(i).Units = string(Units(i)); %update the units to what is defined in the spreadsheet
            else
...
%% Data Importing Function
    function PARAMS = importfile(FileName, sheetName, DatasheetRange)
        ...
        %
        %  Example:
        %  NovSpecies = importfile("/Users/stuartmcknight/Dropbox/SARS-CoV-2-Infection-Computational-Model/Model_2_InnateImmune/SpeciesParameterConventions/12Nov_Species.xlsx", "Sheet1", [2, 47]);
        %
        %  See also READTABLE.
        %
        % Auto-generated by MATLAB on 12-Nov-2020 16:32:29
        ...
        opts.DataRange = "A" + DatasheetRange(1, 1) + ":J" + DatasheetRange(1, 2);
        
        % Specify column names and types
        opts.VariableNames = ["Species_Index", "Species_Name", "Model_Name", "Initial_Amount", "Value", "Constant_Amount", "Boundary_Condition", "Type", "Compartment_Type", "Compartment"];
        opts.VariableTypes = ["string", "string", "string", "string", "string", "categorical", "categorical", "categorical", "categorical", "categorical"];
        
        % Specify variable properties
        opts = setvaropts(opts, ["Species_Index", "Species_Name", "Model_Name", "Initial_Amount", "Value"], "WhitespaceRule", "preserve");
        opts = setvaropts(opts, ["Species_Index", "Species_Name", "Model_Name", "Initial_Amount", "Value", "Constant_Amount", "Boundary_Condition", "Type", "Compartment_Type", "Compartment"], "EmptyFieldRule", "auto");
        
        % Import the data
        PARAMS = readtable(FileName, opts, "UseExcel", false);
        
        for idx = 2:size(DatasheetRange, 1)
            opts.DataRange = "A" + DatasheetRange(idx, 1) + ":J" + DatasheetRange(idx, 2);
